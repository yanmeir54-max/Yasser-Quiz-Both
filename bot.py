...
# ==========================================
# 4. دالة قالب الاسئلة اختيارات
# ==========================================
def build_smart_buttons(quiz, user_id=None):
    buttons = []
    # نستخدم voter_list من الجدول بدلاً من voters
    voters_count_total = len(quiz.get('voter_list', {}))
    
    # التحقق هل اللاعب شارك؟
    has_participated = str(user_id) in quiz.get('voter_list', {})

    # options موجودة في الجدول كـ jsonb
    for i, opt in enumerate(quiz.get('options', [])):
        # نأخذ عدد الأصوات من votes_results في الجدول
        voters_in_this_opt = quiz.get('votes_results', {}).get(str(i), 0)
        
        # حساب النسبة
        percent = round((voters_in_this_opt / voters_count_total * 100)) if voters_count_total > 0 else 0
        
        if has_participated:
            # correct_answer هو العمود في جدولك للإجابة الصح
            if str(opt) == quiz.get('current_answer'):
                prefix = "✅"
            elif quiz.get('voter_list', {}).get(str(user_id)) == i:
                prefix = "❌"
            else:
                prefix = "⚪️"
            btn_text = f"{prefix} {opt} ⦗ {percent}% ⦘"
        else:
            btn_text = f"🔹 {opt}"

        buttons.append([InlineKeyboardButton(text=btn_text, callback_data=f"v_{i}")])
...
async def sync_points_to_global_db(group_scores=None, winners_list=None, cat_name="عام", is_special=False, quiz_id=None):
    """
    👑 محرك ياسر الملكي المتكامل 2026 - نسخة "الصبنة" الممنوعة 😂
    - تدعم الجرد من السجل الرقمي (quiz_id) أو الذاكرة (group_scores)
    - تعالج خطأ 'NoneType' و unexpected keyword argument
    """
    try:
        # 1️⃣ جلب البيانات من سجل الإجابات إذا توفر quiz_id (للمسابقات العامة/الإذاعة)
        if quiz_id:
            try:
                ...
                if res_log.data:
                    ...
                    for row in res_log.data:
                        ...
                        if uid not in group_scores[cid]:
                            group_scores[cid][uid] = {'name': row['user_name'], 'points': 0}
                        group_scores[cid][uid]['points'] += row['points_earned']
                    logging.info(f"📊 تم جلب حصاد {len(res_log.data)} إجابة من السجل الرقمي.")
            except Exception as e:
                logging.error(f"❌ خطأ في سحب بيانات اللوج: {e}")

        # 2️⃣ تأمين المحرك من البيانات الفارغة
        if not group_scores:
            logging.warning("⚠️ لا توجد بيانات للترحيل (المخزن فارغ).")
            return

        # 3️⃣ تحديد المجموعات الفائزة (لحساب الـ wins)
        ...
        # 4️⃣ تجميع حصاد اللاعبين النهائي
        final_tallies = {}
        ...
        for uid, data in final_tallies.items():
            try:
                ...
                logging.info(f"✅ تم ترحيل بيانات البطل: {data['name']}")
            except Exception as e:
...
async def render_final_settings_panel(message, data, owner_id):
    """الدالة الموحدة لعرض لوحة الإعدادات مع الخيارات الجديدة"""
    q_time = data.get('quiz_time', 15)
    q_count = data.get('quiz_count', 10)
    q_mode = data.get('quiz_mode', 'السرعة ⚡')
    # إضافة الخيار الجديد (نمط الاختيارات)
    q_style = data.get('quiz_style', 'أزرار تفاعلية 📊') 
    ...
    # تحديث نص اللوحة ليشمل الخيارات الجديدة
    text = (
    ...
    # 1. أزرار أعداد الأسئلة الجديدة (10 إلى 80)
    kb.row(InlineKeyboardButton("📊 اختر عدد الأسئلة:", callback_data="ignore"))
    counts = [10, 15, 20, 25, 30, 35, 40, 50, 60, 70, 80]
    # تقسيم الأزرار لصفوف لكي لا تخرج عن الشاشة
    row1 = [InlineKeyboardButton(f"{'✅' if q_count==n else ''}{n}", callback_data=f"set_cnt_{n}_{owner_id}") for n in counts[:5]]
    row2 = [InlineKeyboardButton(f"{'✅' if q_count==n else ''}{n}", callback_data=f"set_cnt_{n}_{owner_id}") for n in counts[5:]]
    kb.add(*row1)
    kb.add(*row2)

    # 2. أزرار الوقت الجديدة (10 إلى 60)
    kb.row(InlineKeyboardButton("⏱️ اختر الوقت (ثانية):", callback_data="ignore"))
    times = [10, 15, 20, 30, 45, 60]
    btn_times = [InlineKeyboardButton(f"{'✅' if q_time==n else ''}{n}", callback_data=f"set_time_{n}_{owner_id}") for n in times]
    kb.add(*btn_times)

    # 3. زر "نمط العرض" المتغير (اختيارات أو نص)
    kb.row(InlineKeyboardButton(f"🎨 نمط العرض: {q_style}", callback_data=f"cyc_style_{owner_id}"))
...
# ==========================================
# --- [ معالج قالب الاختيارات ] ---
# ==========================================
@dp.callback_query_handler(lambda c: c.data.startswith('v_'))
async def handle_vote(callback: types.CallbackQuery):
    user_id = str(callback.from_user.id) # سوبابيس يفضل النصوص في المفاتيح
    chat_id = callback.message.chat.id
    opt_idx = int(callback.data.split('_')[1])
    
    # جلب السؤال من الذاكرة أو سوبابيس
    quiz = active_quizzes.get(chat_id)
    
    if not quiz or not quiz.get('is_active'):
        return await callback.answer("⏳ انتهى وقت هذا السؤال!", show_alert=True)

    # 1. منع التكرار (نتحقق من voter_list)
    if user_id in quiz['voter_list']:
        return await callback.answer("🚫 أجبت مسبقاً!", show_alert=True)

    # 2. تحديث البيانات في الذاكرة (للسرعة)
    quiz['voter_list'][user_id] = opt_idx
    quiz['votes_results'][str(opt_idx)] = quiz['votes_results'].get(str(opt_idx), 0) + 1

    # 3. الربط مع سوبابيس (تحديث الجدول)
    # نقوم بتحديث الأعمدة التي أنشأتها في جدولك
    try:
        supabase.table("active_quizzes").update({
            "votes_results": quiz['votes_results'],
            "voter_list": quiz['voter_list']
    ...
    await callback.message.edit_reply_markup(
        reply_markup=build_smart_buttons(quiz, user_id=user_id)
    )
...
async def quiz_settings_engines(c: types.CallbackQuery, state: FSMContext):
    ...
    # 1️⃣ --- قسم المحركات (التعديل اللحظي في الذاكرة State) ---
    if action in ['tog', 'cyc', 'set']:
        ...
        # --- [جديد] محرك نمط العرض (أزرار / نص) ---
        elif action == 'cyc' and data_parts[1] == 'style':
            curr_style = data.get('quiz_style', 'أزرار تفاعلية 📊')
            next_style = "نص عادي 📝" if curr_style == "أزرار تفاعلية 📊" else "أزرار تفاعلية 📊"
            await state.update_data(quiz_style=next_style)
            await c.answer(f"🎨 تم التغيير إلى: {next_style}")
        ...
        # محرك التلميح الموحد
            ...
            await c.answer("💡 مفعل" if not is_on else "💡 معطل")
        
        # --- [تحديث] محرك الوقت المباشر (set_time) ---
        elif action == 'set' and data_parts[1] == 'time':
            new_time = int(data_parts[2])
            await state.update_data(quiz_time=new_time)
            await c.answer(f"⏱️ تم ضبط الوقت: {new_time} ثانية")
        ...
        # --- [تحديث] محرك عدد الأسئلة (set_cnt) ---
        elif action == 'set' and data_parts[1] == 'cnt':
            count_val = int(data_parts[2])
            await state.update_data(quiz_count=count_val)
            await c.answer(f"📊 عدد الأسئلة: {count_val}")

        # تحديث اللوحة فوراً بعد أي تغيير لرؤية النتائج
    ...
    # 2️⃣ --- قسم بدء الحفظ والتشغيل ---
        ...
        # التحقق من الأقسام المختارة (إذا كان لديك نظام فئات)
        if not data.get('selected_cats') and not data.get('is_bot_quiz'):
             # إذا كنت تسمح ببدء مسابقة بدون فئات يدوية، تجاوز هذا الشرط
             pass 
        ...
        if is_broadcast:
            ...
            await c.answer("📍 مسابقة داخلية.")

        # الانتقال لخطوة تسمية المسابقة لحفظها في سوبابيس
        await Form.waiting_for_quiz_name.set() 
        return await c.message.edit_text(
            "📝 **يا بطل، أرسل الآن اسماً لمسابقتك:**\n\n"
            "*(سيتم حفظ إعدادات الوقت، النمط، والنطاق تحت هذا الاسم)*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton("❌ إلغاء التجهيز", callback_data=f"close_{owner_id}")
            )
        )
...
async def process_quiz_name_final(message: types.Message, state: FSMContext):
    ...
    # تجهيز البيانات (Payload) مع إضافة العمود الجديد
    payload = {
        "created_by": u_id,
        "quiz_name": quiz_name,
        "chat_id": u_id,
        ...
        "quiz_style": data.get('quiz_style', 'أزرار تفاعلية 📊'), # السطر الجديد للحفظ
    ...
    try:
        ...
        await state.finish() # تنظيف الذاكرة بعد النجاح
...
async def handle_secure_actions(c: types.CallbackQuery, state: FSMContext):
    try:
        ...
        # 2️⃣ لوحة الإعدادات المطورة
            ...
            q_count = q.get('questions_count', 10)
            q_mode = q.get('mode', 'السرعة ⚡')
            q_style = q.get('quiz_style', 'أزرار تفاعلية 📊') # الخيار الجديد
            ...
            # أزرار عدد الأسئلة (المحدثة: 10 إلى 80)
            kb.row(InlineKeyboardButton("📊 اختر عدد الأسئلة:", callback_data="ignore"))
            counts = [10, 15, 20, 25, 30, 35, 40, 50, 60, 70, 80]
            btn_counts = [InlineKeyboardButton(f"{'✅' if q_count==n else ''}{n}", callback_data=f"set_c_{quiz_id}_{n}_{user_id}") for n in counts]
            # تقسيم الأزرار لصفوف لجمالية الشكل
            kb.add(*btn_counts[:5])
            kb.add(*btn_counts[5:])

            # أزرار الوقت (المحدثة: 10 إلى 60)
            kb.row(InlineKeyboardButton("⏱️ اختر الوقت (ثانية):", callback_data="ignore"))
            times = [10, 15, 20, 30, 45, 60]
            kb.add(*[InlineKeyboardButton(f"{'✅' if q_time==n else ''}{n}", callback_data=f"set_t_{quiz_id}_{n}_{user_id}") for n in times])

            # زر نمط العرض (متغير)
            kb.row(InlineKeyboardButton(f"🎨 نمط العرض: {q_style}", callback_data=f"toggle_style_{quiz_id}_{user_id}"))
            ...
            kb.row(InlineKeyboardButton("🔙 رجوع", callback_data=f"manage_quiz_{quiz_id}_{user_id}"))
        ...
        # أضفنا 'set_t_' للوقت و 'toggle_style_' للنمط الجديد
            ...
            if 'toggle_scope_' in c.data:
                ...
                status_text = "عام 🌐" if new_val else "داخلي 📍"
                await c.answer(f"✅ النطاق: {status_text}")
            ...
            # 💡 محرك التلميح
                ...
                await c.answer("✅ تم تحديث وضع التلميح")

            # 🔖 محرك نظام اللعب (سرعة/وقت كامل)
            ...
            # 🎨 محرك نمط العرض (الزر المتغير الجديد)
            elif 'toggle_style_' in c.data:
                res = supabase.table("saved_quizzes").select("quiz_style").eq("id", quiz_id).single().execute()
                curr_style = res.data.get('quiz_style', 'أزرار تفاعلية 📊')
                new_style = "نص عادي 📝" if curr_style == "أزرار تفاعلية 📊" else "أزرار تفاعلية 📊"
                supabase.table("saved_quizzes").update({"quiz_style": new_style}).eq("id", quiz_id).execute()
                await c.answer(f"✅ النمط: {new_style}")

            # 📊 محرك عدد الأسئلة المباشر (10 إلى 80)
                ...
                await c.answer(f"✅ تم تحديد {count} سؤال")

            # ⏱️ محرك الوقت المباشر (10 إلى 60)
            elif 'set_t_' in c.data:
                time_val = int(data_parts[3])
                supabase.table("saved_quizzes").update({"time_limit": time_val}).eq("id", quiz_id).execute()
                await c.answer(f"✅ المهلة: {time_val} ثانية")

            await c.answer("تم التحديث ✅")
            # إعادة توجيه ذاتي لتحديث الواجهة (هنا يتم عرض q_style الجديد في اللوحة)
            c.data = f"quiz_settings_{quiz_id}_{user_id}"
            return await handle_secure_actions(c, state)
        
        # 4️⃣ تغيير الوقت (النمط القديم - أبقيناه للتوافق)
        ...
        # 5️⃣ الحفظ والعمليات النهائية
        elif c.data.startswith('save_quiz_process_'):
            quiz_id = data_parts[3] 
            await c.answer("✅ تم حفظ جميع الإعدادات!", show_alert=True)
