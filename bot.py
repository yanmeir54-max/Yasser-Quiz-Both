...
# 2️⃣ تعديل لوحة الأزرار (إضافة زر "إضافة دولتي")
...
async def process_bank_transfer(sender_id, amount, receiver_id=None, receiver_acc=None):
    try:
        ...
        msg += f"📉 <b>الـعـمـولـة (5%) :</b> <code>{fee}</code> ن\n"
...
# ============================================================
@dp.callback_query_handler(lambda c: c.data and c.data.startswith('set_country_'))
async def open_countries_menu(callback_query: types.CallbackQuery):
    # استخراج الـ user_id من الـ callback_data
    user_id = int(callback_query.data.split('_')[2])
    
    # التأكد أن الشخص الذي ضغط هو صاحب البروفايل (لحماية الخصوصية)
    if callback_query.from_user.id != user_id:
        return await callback_query.answer("⚠️ عذراً، هذا الزر مخصص لصاحب الحساب فقط!", show_alert=True)

    # 1. استدعاء دالة لوحة الدول التي أنشأناها سابقاً
    countries_kb = get_countries_keyboard(user_id)
    
    # 2. تعديل الرسالة الحالية لتظهر فيها قائمة الدول
    await callback_query.message.edit_text(
        text="<b>🌍 | يرجى اختيار دولتك من القائمة أدناه ليتم تحديث بروفايلك:</b>",
        reply_markup=countries_kb,
        parse_mode="HTML"
    )
    
    # إغلاق ساعة التحميل في التلجرام
...
async def engine_global_broadcast(chat_ids, quiz_data, owner_name, current_quiz_db_id=None):
    ...
    try:
        ...
        try:
            # تحديث عدد الأعضاء في الجدول العالمي لكل مجموعة شاركت
            for cid, scores in group_scores.items():
                active_members_count = len(scores)
                # تحديث عداد الأعضاء في قاعدة البيانات
