...
def build_smart_buttons(quiz, user_id=None):
    ...
    # تم حذف سطر زر "من اختار ماذا؟" من هنا ليكون الشكل أنظف
...
async def handle_vote(callback: types.CallbackQuery):
    ...
    # 1. محاولة جلب السؤال من الذاكرة
    quiz = active_quizzes.get(chat_id)
    
    # 2. [إصلاح ذكي] إذا لم يوجد في الذاكرة، نبحث عنه في سوبابيس
    if not quiz:
        try:
            response = supabase.table("active_quizzes").select("*").eq("chat_id", chat_id).eq("is_active", True).execute()
            if response.data:
                quiz = response.data[0]
                # نعيد تخزينه في الذاكرة لسرعة الضغطات القادمة
                active_quizzes[chat_id] = quiz
        except Exception as e:
            print(f"Error fetching from Supabase: {e}")

    # 3. التحقق النهائي من وجود السؤال ونشاطه
    if not quiz or not quiz.get('is_active'):
        return await callback.answer("⏳ انتهى وقت هذا السؤال أو تم إغلاقه!", show_alert=True)

    # 4. منع التكرار
    if user_id in quiz.get('voter_list', {}):
        return await callback.answer("🚫 أجبت مسبقاً يا بطل!", show_alert=True)

    # 5. تحديث البيانات (تأكد أن القواميس ليست None)
    if 'voter_list' not in quiz or quiz['voter_list'] is None: quiz['voter_list'] = {}
    if 'votes_results' not in quiz or quiz['votes_results'] is None: quiz['votes_results'] = {}

    quiz['voter_list'][user_id] = opt_idx
    quiz['votes_results'][str(opt_idx)] = quiz['votes_results'].get(str(opt_idx), 0) + 1

    # 6. التحديث الفوري في سوبابيس
    try:
    ...
    # 7. تحديث الأزرار للمستخدم الحالي
    try:
        ...
        # لتجنب خطأ في حال حاول تليجرام تعديل رسالة لم تتغير
        await callback.answer("✅ تم تسجيل إجابتك")
...
async def run_universal_logic(chat_id, questions, quiz_data, owner_name, engine_type):
    ...
    questions  # السلة التي سيسحب منها الخيارات
        )
