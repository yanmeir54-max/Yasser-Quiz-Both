...
# ==========================================
# 4. دالة قالب الاسئلة اختيارات (بدون زر الإحصائيات)
# ==========================================

def get_official_quiz_style(q_data, current_index, total_q, quiz_db_id, cat_name):
    """
    محاكاة دقيقة جداً لشكل بوت تلجرام الرسمي @QuizBot (نسخة معدلة بالملي)
    """
    question_text = q_data.get('question_text', '') or q_data.get('question', '')
    options = q_data.get('options', [])
    
    # 1. الترقيم العلوي: خط عريض للرقم الحالي وعادي للإجمالي
    header = f"<b>{current_index} من {total_q}</b>"
    
    # 2. جسم السؤال: القسم (بدلاً من مجهول) ثم السؤال
    body = (
        f"<i>اختبار: {cat_name}</i>\n\n"
        f"❓ <b>{question_text}</b>"
    )
    
    # 3. بناء الأزرار: خيار واحد في كل سطر لضمان العرض الكامل
    markup = InlineKeyboardMarkup(row_width=1)
    
    for idx, opt in enumerate(options):
        # Callback Data مشفر لضمان دقة النتائج
        callback_data = f"ans_{quiz_db_id}_{current_index}_{idx}"
        
        # إضافة الزر (النص فقط كما في @QuizBot الرسمي)
        markup.add(InlineKeyboardButton(text=str(opt), callback_data=callback_data))
    
    # دمج المكونات مع سطر فارغ دقيق بين الهيدر والبودي
    final_text = f"{header}\n\n{body}"
...
async def send_quiz_question_with_official_markup(chat_id, q_data, current_num, total_num, settings, markup):
    """
    إرسال السؤال بتنسيق مطابق لـ @QuizBot (بالملي)
    """
    q_text = q_data.get('question_content') or q_data.get('question_text') or "⚠️ نص السؤال مفقود!"
    cat_name = settings.get('cat_name', 'عام')

    # بناء النص (المحاكاة الدقيقة)
    # 1. الترقيم (Bold)
    # 2. نوع الاختبار (Italic)
    # 3. السؤال (Bold)
    text = (
        f"<b>{current_num} من {total_num}</b>\n\n"
        f"<i>اختبار: {cat_name}</i>\n\n"
        f"❓ <b>{q_text}</b>"
    )
    ...
    try:
        return await bot.send_message(
            ...
            text, 
            reply_markup=markup, 
            parse_mode='HTML' # ضروري جداً لتشغيل وسوم b و i
        )
    except Exception as e:
...
async def get_ultra_smart_options(question_text, category_name, correct_ans):
    try:
        ...
        # 1️⃣ [نظام الرأس المتخصص]: (نهر، خليج، بحر، قانون، مدينة)
        # إذا بدأت الإجابة بكلمة مفتاحية قوية، نكتفي بها تماماً لضمان طقم موحد
        if len(ans_words) > 1:
            head_word = ans_words[0]
            if len(head_word) >= 3:
                res = supabase.table("bot_questions").select("correct_answer") \
                    .ilike("correct_answer", f"{head_word}%") \
                ...
                # شرط الحصانة: إذا وجدنا 3 خيارات من نفس "الرأس"، نخرج فوراً لمنع الاختلاط
                if len(fakes) >= 3: return random.sample(fakes, 3)

        # 2️⃣ [نظام قافية الكلمة الواحدة]: (أول حرفين + آخر حرفين)
        # إذا كانت الإجابة كلمة واحدة (آسيا، عمان)، يبحث عن تطابق صوتي في البداية والنهاية
        ...
        # 3️⃣ [نظام التشظي للإجابات الطويلة]: (أول 3 حروف + آخر 3 حروف)
        if len(correct_ans) > 7:
        ...
        # 4️⃣ [رادار نوع السؤال]: (ما هي الدولة، ما هي القارة)
        # يبحث عن أسئلة تبدأ بنفس الصيغة ويجلب إجاباتها
        q_start = " ".join(re.findall(r'\w+', q_norm)[:3]) # أول 3 كلمات من السؤال
        if q_start:
            res = supabase.table("bot_questions").select("correct_answer") \
                .ilike("question_content", f"{q_start}%").limit(15).execute()
        ...
        # 5️⃣ [فحص الكلمات المفتاحية الذكي]: (البداية والنهاية للسؤال)
        q_words = re.findall(r'\w+', q_norm)
        if len(q_words) >= 4:
            # نستبعد الحشو ونأخذ كلمات الجوهر
            ignored = [normalize_arabic(w) for w in ['ما', 'هي', 'اين', 'التي', 'الذي']]
            keys = [w for w in q_words if w not in ignored and len(w) > 3]
            if keys:
                best_k = keys[0] if len(keys) == 1 else f"{keys[0]},{keys[-1]}"
                res = supabase.table("bot_questions").select("correct_answer") \
                    .or_(f"question_content.ilike.%{keys[0]}%,question_content.ilike.%{keys[-1]}%").limit(10).execute()
        ...
        # 6️⃣ [خطة الطوارئ]: إذا لم يكتمل العدد، نسحب من نفس القسم مع شرط "الـ" التعريف
        if len(fakes) < 3:
            res = supabase.table("bot_questions").select("correct_answer") \
...
async def process_quiz_answer(callback_query: types.CallbackQuery):
    ...
    # 2. جلب بيانات المسابقة من الرادار
    quiz = active_quizzes.get(chat_id)
    
    # 🔥 القفل الفوري: إذا توقف المحرك أو انتهى الوقت، ارفض الضغطة فوراً
    if not quiz or quiz.get('active') == False:
        return await callback_query.answer("🚫 انتهى وقت استقبال الإجابات لهذا السؤال!", show_alert=True)

    # 3. التأكد من أن الزر ينتمي للمسابقة الحالية (منع التداخل)
    if quiz.get('quiz_id') != q_id_from_button:
        return await callback_query.answer("⚠️ هذه الأزرار منتهية الصلاحية.", show_alert=True)

    # 4. قفل المحاولة الواحدة (الذي أصلحناه سابقاً)
    if 'voted_users' not in quiz: quiz['voted_users'] = []
    if user_id in quiz['voted_users']:
        return await callback_query.answer("لديك محاولة واحدة فقط! 🧐", show_alert=False)

    # 5. تسجيل الضغطة (بصمة المستخدم)
    quiz['voted_users'].append(user_id)

    # 6. التحقق من الإجابة (بالملي)
    try:
        ...
        if selected_text == correct_ans:
            quiz['winners'].append({'id': user_id, 'name': callback_query.from_user.full_name})
            await callback_query.answer("إجابة صحيحة! 🎉", show_alert=False)
        else:
            await callback_query.answer("إجابة خاطئة! ❌", show_alert=False)
    except:
        await callback_query.answer("خطأ في معالجة الإجابة.")
